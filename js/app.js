...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h5><b>Rating: </b> ${product.rating.rate}</h5>
      <h5><b>Reviews: </b> ${product.rating.count}</h5>
      <h2>Price: $ ${product.price}</h2>
      <br>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn-custom">add to cart</button>
      <button id="details-btn" class="btn-danger-custom">Details</button></div>
      `;
